...
extern int iface;
extern uint16_t vlan;
extern uint8_t l3_tos;

extern uint16_t l2_hdr_size;
extern uint16_t l3_hdr_size;
extern uint16_t l4_hdr_size;
extern uint16_t dhcp_hdr_size;
extern uint16_t fqdn_n;
extern uint16_t fqdn_s;

extern uint8_t dhcp_packet_disc[1514];
extern uint8_t dhcp_packet_offer[1514];
extern uint8_t dhcp_packet_request[1514];
extern uint8_t dhcp_packet_ack[1514];
extern uint8_t dhcp_packet_release[1514];
extern uint8_t dhcp_packet_decline[1514];

extern uint8_t dhopt_buff[500];
extern uint32_t dhopt_size;
extern uint32_t dhcp_xid;
extern uint32_t bcast_flag;
extern uint8_t timeout;
extern uint8_t padding_flag;
extern uint8_t vci_buff[256];
extern uint8_t hostname_buff[256];
extern uint8_t fqdn_buff[256];
extern uint32_t option51_lease_time;
extern uint32_t port;
extern uint8_t unicast_flag;
extern uint8_t nagios_flag;
extern uint8_t json_flag;
extern uint8_t json_first;
extern uint8_t *giaddr;
extern uint8_t *server_addr;

extern uint8_t no_custom_dhcp_options;
...
extern uint8_t *dhopt_pointer_g;

struct arp_hdr *arp_hg;
struct icmp_hdr *icmp_hg;

extern uint8_t dhmac[ETHER_ADDR_LEN];
extern uint8_t dmac[ETHER_ADDR_LEN];
extern uint8_t iface_mac[ETHER_ADDR_LEN];

extern char dhmac_fname[20];
extern char iface_name[30];
extern char ip_str[128];
extern uint32_t server_id, option50_ip;
extern uint8_t dhcp_release_flag;

extern uint32_t unicast_ip_address;
extern uint32_t ip_address;
extern uint8_t ip_listen_flag;
extern uint8_t arp_icmp_packet[1514];
extern uint8_t arp_icmp_reply[1514];
extern uint16_t icmp_len;
extern struct timeval tval_listen;
extern uint32_t listen_timeout;
 ...
 */
uint32_t get_interface_address()
{
    int status;
    struct ifreq ifr;
...
int print_buff(uint8_t *buff, int size)
{
    int tmp;
    fprintf(stdout, "\n---------Buffer data---------\n");
 ...
 */
uint16_t ipchksum(uint16_t *buff, int words)
{
    unsigned int sum, i;
    sum = 0;
    for (i = 0; i < words; i++)
 ...
 */
uint16_t icmpchksum(uint16_t *buff, int words)
{
    unsigned int sum, i;
    ...
    if ((icmp_len % 2) == 1)
    {
        last_word = *((uint8_t *)buff + icmp_len + ICMP_H - 1);
 ...
 */
uint16_t l4_sum(uint16_t *buff, int words, uint16_t *srcaddr, uint16_t *dstaddr, uint16_t proto, uint16_t len)
{
    ...
    if ((htons(len) % 2) == 1)
    {
        last_word = *((uint8_t *)buff + ntohs(len) - 1);
        last_word = (htons(last_word) << 8);
        sum = 0;
        for (i = 0; i < words; i++)
        {
...
int build_option53(int msg_type)
{
    ...
    {
        uint8_t msgtype = DHCP_MESSAGETYPE;
        uint8_t msglen = 1;
        uint8_t msg = DHCP_MSGDISCOVER;
...
int build_option50()
{
    uint8_t msgtype = DHCP_REQUESTEDIP;
    uint8_t msglen = 4;
    uint32_t msg = option50_ip;
...
int build_option51()
{
    uint8_t msgtype = DHCP_LEASETIME;
    uint8_t msglen = 4;
    uint32_t msg = htonl(option51_lease_time);
...
int build_option54()
{
    uint8_t msgtype = DHCP_SERVIDENT;
    uint8_t msglen = 4;
    uint32_t msg = server_id;
...
{
    uint32_t msgtype = DHCP_CLASSSID;
    uint32_t msglen = strlen((const char *)vci_buff);
...
{
    uint32_t msgtype = DHCP_HOSTNAME;
    uint32_t msglen = strlen((const char *)hostname_buff);
...
{
    uint32_t msgtype = DHCP_FQDN;
    uint8_t flags = 0;
    uint8_t rcode1 = 0;
    uint8_t rcode2 = 0;
    uint32_t msglen = strlen((const char *)fqdn_buff) + 3;
...
int build_custom_dhcp_options()
{
    ...
    for (option_index = 0; option_index < no_custom_dhcp_options; option_index++)
    {
        ...
        uint8_t msgtype = custom_dhcp_options[option_index].option_no;
        uint8_t msglen = custom_dhcp_options[option_index].option_len;
        uint8_t option_type = custom_dhcp_options[option_index].option_type;
...
{
    uint32_t dhcp_packet_size = dhcp_hdr_size + dhopt_size;
    if (!dhcp_release_flag)
    {
        uint8_t dmac_tmp[ETHER_ADDR_LEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    ...
    if (pkt_type == DHCP_MSGDISCOVER)
    {
        ...
        iph->check = ipchksum((uint16_t *)(dhcp_packet_disc + l2_hdr_size), iph->ihl << 1);

        struct udphdr *uh = (struct udphdr *)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size);
        uh->source = htons(port + 1);
        uh->dest = htons(port);
        uint16_t l4_proto = 17;
        uint16_t l4_len = (l4_hdr_size + dhcp_hdr_size + dhopt_size);
        uh->len = htons(l4_len);
        ...
        uint8_t *dhopt_pointer = (uint8_t *)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum((uint16_t *)(dhcp_packet_disc + l2_hdr_size + l3_hdr_size), ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t *)&iph->saddr, (uint16_t *)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
    if (pkt_type == DHCP_MSGREQUEST)
    {
        ...
        if (unicast_flag)
            ...
            iph->saddr = inet_addr("0.0.0.0");
        iph->daddr = inet_addr((const char *)server_addr);
        iph->check = ipchksum((uint16_t *)(dhcp_packet_request + l2_hdr_size), iph->ihl << 1);
        ...
        /* DHCP option buffer is copied here to DHCP packet */
        uint8_t *dhopt_pointer = (uint8_t *)(dhcp_packet_request + l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum((uint16_t *)(dhcp_packet_request + l2_hdr_size + l3_hdr_size), ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t *)&iph->saddr, (uint16_t *)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
    if (pkt_type == DHCP_MSGRELEASE)
    {
        ...
        iph->check = ipchksum((uint16_t *)(dhcp_packet_release + l2_hdr_size), iph->ihl << 1);
        ...
        /* DHCP option buffer is copied here to DHCP packet */
        uint8_t *dhopt_pointer = (uint8_t *)(dhcp_packet_release + l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum((uint16_t *)(dhcp_packet_release + l2_hdr_size + l3_hdr_size), ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t *)&iph->saddr, (uint16_t *)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
    ...
    if (pkt_type == DHCP_MSGDECLINE)
    {
        ...
        iph->check = ipchksum((uint16_t *)(dhcp_packet_decline + l2_hdr_size), iph->ihl << 1);
        ...
        /* DHCP option buffer is copied here to DHCP packet */
        uint8_t *dhopt_pointer = (uint8_t *)(dhcp_packet_decline + l2_hdr_size + l3_hdr_size + l4_hdr_size + dhcp_hdr_size);
        memcpy(dhopt_pointer, dhopt_buff, dhopt_size);

        /* UDP checksum is done here */
        uh->check = l4_sum((uint16_t *)(dhcp_packet_decline + l2_hdr_size + l3_hdr_size), ((dhcp_hdr_size + dhopt_size + l4_hdr_size) / 2), (uint16_t *)&iph->saddr, (uint16_t *)&iph->daddr, htons(l4_proto), htons(l4_len));
    }
...
int build_packet(int pkt_type)
{
    ...
    if (pkt_type == ARP_SEND)
    {
        ...
        memcpy(arph->sender_ip, (uint8_t *)&ip_addr_tmp, ETHER_ADDR_LEN);
    ...
    else if (ICMP_SEND)
    {
        ...
        memcpy((((uint8_t *)&ich->seq) + 1), (((uint8_t *)&icmp_hg->seq) + 1), (icmp_len + 1));
        iph->tot_len = htons((l3_hdr_size + ICMP_H + icmp_len));
        iph->check = ipchksum((uint16_t *)(arp_icmp_reply + l2_hdr_size), iph->ihl << 1);
        ich->icmp_sum = icmpchksum((uint16_t *)(arp_icmp_reply + l2_hdr_size + l3_hdr_size), ((icmp_len + ICMP_H) / 2));
    }
    return 0;
}
...
int check_packet(int pkt_type)
{
    ...
    else if (pkt_type == ARP_ICMP_RCV)
    {
        ...
        if (!vlan)
        {
            ...
            if ((ntohs(arp_hg->ar_op)) == ARPOP_REQUEST && (htonl(ip_address)) == (*((uint32_t *)(arp_hg->target_ip))))
            {
                return ARP_RCVD;
            }
        }
...
int set_serv_id_opt50()
{
    ...
    while (*(dhopt_pointer_g) != DHCP_END)
    {
        ...
        {
            memcpy(&server_id, (uint32_t *)(dhopt_pointer_g + 2), 4);
        }
...
int print_dhinfo(int pkt_type)
{
    ...
    while (*(dhopt_pointer_g) != DHCP_END)
    {
        ...
        case DHCP_SERVIDENT:
            if (json_flag)
            {
                ...
                fprintf(stdout, "{\"msg\":\"DHCP server - %s\","
                                ...
                                "\"result-ip\":\"%s\"}",
                        get_ip_str(*(uint32_t *)(dhopt_pointer_g + 2)),
                        get_ip_str(*(uint32_t *)(dhopt_pointer_g + 2)));
            }
            else
            {
        ...
        case DHCP_LEASETIME:
            if (json_flag)
            {
                ...
                fprintf(stdout, "{\"msg\":\"Lease time - %d Days %d Hours %d Minutes\","
                                ...
                                "\"result-leasetime\":\"%d\"}",
                        (ntohl(*(uint32_t *)(dhopt_pointer_g + 2))) / (3600 * 24),
                        ((ntohl(*(uint32_t *)(dhopt_pointer_g + 2))) % (3600 * 24)) / 3600,
                        (((ntohl(*(uint32_t *)(dhopt_pointer_g + 2))) % (3600 * 24)) % 3600) / 60,
                        ntohl(*(uint32_t *)(dhopt_pointer_g + 2)));
            }
            else
            {
            ...
            for (tmp = 0; tmp < (*(dhopt_pointer_g + 1) / 4); tmp++)
            {
                if (json_flag)
                {
                    ...
                    fprintf(stdout, "{\"msg\":\"Subnet mask - %s\","
                                    ...
                                    "\"result-router\":\"%s\"}",
                            get_ip_str(*(uint32_t *)(dhopt_pointer_g + 2 + (tmp * 4))),
                            get_ip_str(*(uint32_t *)(dhopt_pointer_g + 2 + (tmp * 4))));
                }
                else
                {
        ...
        case DHCP_FQDN:
        {
            ...
            uint32_t size = (uint32_t) * (dhopt_pointer_g + 1) - 3;
            /* Plus 2 to add string terminator */
            uint8_t fqdn_client_name[size + 1];
...
int map_all_layer_ptr(int pkt_type)
{
    if (pkt_type == DHCP_MSGOFFER && vlan != 0)
    {
        ...
        dhopt_pointer_g = (uint8_t *)(dhcp_packet_offer + l2_hdr_size + l3_hdr_size + l4_hdr_size + sizeof(struct dhcpv4_hdr));
    }
    ...
    else if (pkt_type == DHCP_MSGACK && vlan != 0)
    {
        ...
        dhopt_pointer_g = (uint8_t *)(dhcp_packet_ack + l2_hdr_size + l3_hdr_size + l4_hdr_size + sizeof(struct dhcpv4_hdr));
    }
    else if (pkt_type == DHCP_MSGACK && vlan == 0)
    {
...
{
    FILE *dh_file;
    uint8_t aux_dmac[ETHER_ADDR_LEN + 3]; //[Fix for Seg Fault] @inov8shn - add a few extra chars to this buffer, since fscanf("%2X") below returns a uint32_t, and was triggering a Segmentation Fault when run for last byte read (&aux_dmac[5]).
    ...
    items = fscanf(dh_file, "Client_mac: %s\nAcquired_ip: %s\nServer_id: %s\n\
            Host_mac: %2X:%2X:%2X:%2X:%2X:%2X\nIP_listen: %s Pid: %d",
                   ...
                   (uint32_t *)&aux_dmac[0], (uint32_t *)&aux_dmac[1], (uint32_t *)&aux_dmac[2],
                   (uint32_t *)&aux_dmac[3], (uint32_t *)&aux_dmac[4], (uint32_t *)&aux_dmac[5],
...
int print_dhoption(uint8_t *buff, int size)
{
    int tmp;
    fprintf(stdout, "\n  OPTION data (HEX)\n    ");
    for (tmp = 0; tmp < size; tmp++)
