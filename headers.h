...
int print_buff(uint8_t *buff, int size);     /* Debug routine */
int print_dhoption(uint8_t *buff, int size); /* Print dhcp option */
...
char *get_ip_str(uint32_t ip);               /* Convert in_addr to string */
uint32_t get_interface_address();            /* Return the IP address of the interface. */
...
#define CUST_DHCP_OPTION_IP 3
    uint8_t option_value[256];
    uint32_t option_value_num;
    uint32_t option_value_ip;
};
