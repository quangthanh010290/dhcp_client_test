...
uint8_t dhcp_packet_disc[1518] = {0};
uint8_t dhcp_packet_offer[1518] = {0};
uint8_t dhcp_packet_request[1518] = {0};
uint8_t dhcp_packet_ack[1518] = {0};
uint8_t dhcp_packet_release[1518] = {0};
uint8_t dhcp_packet_decline[1518] = {0};

uint8_t dhopt_buff[500] = {0};
uint32_t dhopt_size = {0};
uint8_t dhmac[ETHER_ADDR_LEN] = {0};
uint8_t dmac[ETHER_ADDR_LEN];
uint8_t iface_mac[ETHER_ADDR_LEN] = {0};
...
uint8_t no_custom_dhcp_options = {0};
struct custom_dhcp_option_hdr custom_dhcp_options[MAX_CUSTOM_DHCP_OPTIONS];

char dhmac_fname[20];
char iface_name[30] = {0};
char ip_str[128];
uint8_t dhmac_flag = 0;
uint8_t strict_mac_flag = 0;
uint32_t server_id = {0}, option50_ip = {0};
uint32_t dhcp_xid = 0;
uint16_t bcast_flag = 0;     /* DHCP broadcast flag */
uint8_t vci_buff[256] = {0}; /* VCI buffer*/
uint16_t vci_flag = 0;
uint8_t hostname_buff[256] = {0}; /* Hostname buffer*/
uint16_t hostname_flag = 0;
uint8_t fqdn_buff[256] = {0}; /* FQDN buffer*/
uint16_t fqdn_flag = 0;
uint16_t fqdn_n = 0;
uint16_t fqdn_s = 0;
uint32_t option51_lease_time = 0;
uint32_t port = 67;
uint8_t unicast_flag = 0;
uint8_t nagios_flag = 0;
uint8_t json_flag = 0;
uint8_t dhcp_decline_flag = 0;
uint8_t json_first = 1;
char *giaddr = "0.0.0.0";
...
uint8_t *dhopt_pointer_g = {0};
uint8_t verbose = 0;
uint8_t dhcp_release_flag = 0;
uint8_t padding_flag = 0;
uint16_t timeout = 0;
...
uint32_t unicast_ip_address = 0;
uint32_t ip_address;
uint8_t ip_listen_flag = 0;
struct timeval tval_listen = {3600, 0};
uint32_t listen_timeout = 3600;
uint8_t arp_icmp_packet[1514] = {0};
uint8_t arp_icmp_reply[1514] = {0};
uint16_t icmp_len = 0;
...
int main(int argc, char *argv[])
{
    ...
    while (get_tmp < argc)
    {
        ...
        case 'm':
        {
            uint8_t aux_dhmac[ETHER_ADDR_LEN + 1];
            ...
            sscanf((char *)optarg, "%2X:%2X:%2X:%2X:%2X:%2X",
                   (uint32_t *)&aux_dhmac[0], (uint32_t *)&aux_dhmac[1],
                   (uint32_t *)&aux_dhmac[2], (uint32_t *)&aux_dhmac[3],
                   (uint32_t *)&aux_dhmac[4], (uint32_t *)&aux_dhmac[5]);
        ...
        case 'c':
            ...
            uint32_t option_value_num = {0}, option_value_ip = {0};
            int option_index = 0;

            if ((sscanf((char *)optarg, "%u,str,%255[^\n]s", (uint32_t *)&option_no, option_value)) == 2)
            {
            ...
            else if ((sscanf((char *)optarg, "%u,num,%u", (uint32_t *)&option_no, &option_value_num)) == 2)
            {
                no_custom_dhcp_options = no_custom_dhcp_options + 1;
                option_index = no_custom_dhcp_options - 1;
            ...
            else if ((sscanf((char *)optarg, "%u,hex,%254s", (uint32_t *)&option_no, option_value)) == 2)
            {
            ...
            else if ((sscanf((char *)optarg, "%u,ip,%s", (uint32_t *)&option_no, option_value)) == 2)
            {
                no_custom_dhcp_options = no_custom_dhcp_options + 1;
                option_index = no_custom_dhcp_options - 1;
